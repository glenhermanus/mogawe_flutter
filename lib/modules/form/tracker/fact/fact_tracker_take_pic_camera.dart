...
class _FactTrackerTakeCameraPicState extends State<FactTrackerTakeCameraPic> {
  ...
  Future _getImageCamera() async {
    final pickedFile = await picker.pickImage(source: imgPicker.ImageSource.camera);
    if (pickedFile != null) {
      setState(() {
        photo = File(pickedFile.path);
        path = photo.path.split('/').last;

        ResultFilePath resultFilePath = new ResultFilePath(path: path);
